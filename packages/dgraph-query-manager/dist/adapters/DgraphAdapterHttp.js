...
class DgraphAdapterHttp {
    ...
    async alterSchema(schema) {
        return this.client
            ...
            .catch(error => {
            logger_1.default.error(`Dgraph schema alteration failed, error: %s`, error);
            return false;
        });
    }
    /**
    ...
    async mutate(serialization, mutationType = MutationTypes_1.MutationTypes.SetJson, commitNow = false) {
        if (serialization.request === undefined) {
            throw Error(`DgraphAdapterHttp.mutate error, payload undefined for data: ${serialization.data}`);
        }
        ...
        try {
            ...
            Object.entries(assigned.data.uids).forEach(([key, uid]) => uids.push(uid));
        }
        catch (e) {
            logger_1.default.error('DgraphAdapterHttp.mutate, payload: %o, mutationType: %o, error: %o', serialization, mutationType, e);
        }
        finally {
    ...
    async queryWithVars(serialization, vars) {
        ...
        try {
            ...
            logger_1.default.error('DgraphAdapterHttp.queryWithVars, query: %s, paramTypes: %o, error: %o', serialization, vars, e);
        }
        finally {
            await transaction.discard();
        }
